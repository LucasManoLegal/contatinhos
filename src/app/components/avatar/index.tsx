...
type Props = { /*definimos uma tipagem separada, que denominamos Props*/
    name: string
    image?: ImageProps | null /*definimos que imagem é opcional
    e pode ser null */
    variant?: "medium" | "large" /*opcional, medium
    ou large*/
}

export function Avatar({ image, name,
    variant="medium" }: Props){ /*caso não seja
    fornecida uma variant, o padrão é a medium*/
    return (
    <View>
        {  image? (
            ...
            <View style={[styles.letter, variants.size[variant]]}>
                <Text style={[styles.text,variants.text[variant]]}>{name
            [0].toUpperCase()}</Text>
            </View>
        )}
    </View>
